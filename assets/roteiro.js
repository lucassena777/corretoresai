...
const roteiro = (() => {
  ...
  // O modelo às vezes escapa a quebra de linha duas vezes e o texto chega com
  // "\n" literal no meio da frase — dois caracteres, não uma quebra. Na tela
  // isso aparece cru, dentro do roteiro que o corretor ia gravar. Desfazemos
  // aqui, na entrada, em vez de no servidor: assim vale para qualquer provedor
  // de IA que venha depois, sem republicar função nenhuma.
  ...
  async function gerarIdeiasIA(dados, perfil, { timeoutMs = 90000 } = {}) {
    ...
    const ideias = ANGULOS.map((angulo, n) => {
      ...
      for (const campo of CAMPOS) {
        const valor = typeof bruta[campo] === "string" ? limpar(bruta[campo]) : "";
